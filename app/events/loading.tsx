...
const Loading = () => {
  const loadingEvents = Array.from({ length: 4 }).map((_, index) => ({
    ...
    images: [<Skeleton key={`image_1_${index}`} height={150} width={200} />, <Skeleton key={`image_2_${index}`} height={150} width={200} />],
  }));
        ...
        {loadingEvents.map((event) => (
          <div key={event._id} className="bg-white shadow-md rounded-lg p-6 mb-6 w-full">
            ...
            <div className="flex flex-col justify-center mt-3 sm:mt-4">
              <div className="grid w-full grid-cols-4 gap-2 sm:gap-4">
                {event.images.map((image, index) => (
                  <div key={`image_container_${event._id}_${index}`} className="aspect-w-1 aspect-h-1">
                    {image}
                  </div>
                ))}
              </div>
            ...
            <div className="flex items-center justify-center mx-4 mb-2 space-x-2">
              <Skeleton key={`button_1_${event._id}`} width={80} height={30} />
              <Skeleton key={`button_2_${event._id}`} width={80} height={30} />
              <Skeleton key={`button_3_${event._id}`} width={80} height={30} />
            </div>
          </div>
        ))}
      </div>

...
const EditEquipmentPage = ({ params }: any) => {
  ...
  useEffect(() => {
    const fetchEquipment = async () => {
      try {
        const response = await fetch(`/api/VCLAB/EquipmentsRoute/${equipmentId}`, {
          method: 'GET',
          headers: {
  ...
  const handleResearchTestsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value && !equipment.researchTests.includes(value)) {
      setEquipment({ ...equipment, researchTests: [...equipment.researchTests, value] });
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        ...
        <input
          ...
          value={equipment.installationDate} required
          onChange={(e) => setEquipment({ ...equipment, installationDate: e.target.value })}
          className="border border-gray-300 px-4 py-2 w-full rounded-md mb-4"
        />
        <div className="mb-4">
          <input
            ...
            <div className="flex flex-wrap gap-2">
              {equipment.researchTests.map((test, index) => (
                <span key={index} className="bg-gray-200 px-2 py-1 rounded-full text-sm">
                  {test} <button onClick={() => setEquipment({ ...equipment, researchTests: equipment.researchTests.filter((_, i) => i !== index) })}>&times;</button>
                </span>
              ))}
            </div>
          )}
      ...
      <Link href="/VCLAB/Equipments" onClick={handleEditEquipment} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-md cursor-pointer">
        Save Changes
      </Link>
    </div>
  );
